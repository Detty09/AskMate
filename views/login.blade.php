...
@section('content')
    <div class="flex flex-col mx-auto p-6 mt-6 border-4 rounded-lg border-gray-900 items-center">
        <x-header>Welcome Back!</x-header>
        <form action="/login" method="POST" class="max-w-md mx-auto mt-10">
            @csrf
            <div class="relative z-0 w-full mb-5 group">
                <input type="email" name="email" id="email"
                       class="block py-2.5 px-0 w-full text-sm border-0 border-b-2  appearance-none text-white border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-0 peer"
